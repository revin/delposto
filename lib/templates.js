...
templates.copySupport = function (targetDir, callback) {
    Object.keys(templates.support).forEach(function (name) {
        var filesCopied = file.copyDir(path.join(templateDir, name), path.join(targetDir, name));
        if (filesCopied && callback) {
            filesCopied.forEach(callback);
        }
    });
};
