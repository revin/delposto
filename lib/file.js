...
file = {
    ...
    makeCompressedCopy: function (srcFileName, destFileName) {
        //make a gzipped copy of a file with the same timestamp as the original
        destFileName = destFileName || srcFileName + '.gz';
        var command = 'gzip -9c ' + srcFileName + ' > ' + destFileName + ' && touch -r ' + srcFileName + ' ' + destFileName;
        exec(command, function(error, stdout, stderr) {
            if (error != null) {
                console.warn('WARNING: Error generating pre-compressed file ' + destFileName);
            }
        });
    }
};
